...
结合近期可追溯消息、Binance U 本位永续真实行情及只读账户余额筛选候选币，并给出由用户自行执行的进攻型市价/近价限价开仓、逐仓/全仓模式、仓位、杠杆、止盈和止损计划。支持以月收益 100%+ 作为研究目标，但不承诺收益，并通过硬性回撤闸门约束尾部风险
...
- 先查项目方、交易所、监管机构等公开消息，再用 4h / 1h / 15m 技术结构确认是否已被市场定价
...
- 可在用户明确授权后以 `0600` 权限保存只读 Binance 密钥到 `~/.crypto`，查询余额、持仓和风险暴露，但绝不调用下单接口
...
- 所有开仓和平仓操作均由用户自行完成，Skill 只记录用户明确反馈的真实成交
...
首次使用时，让 Agent 执行 `python3 scripts/crypto_memory.py init` 初始化 `~/.crypto`。需要加入账户风险判断时，再执行 `python3 scripts/crypto_memory.py configure-binance`，通过隐藏输入保存只读 Binance 凭证；请在 Binance 后台关闭交易和提现权限，并限制可信 IP。
