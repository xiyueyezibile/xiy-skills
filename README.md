...
Skill 包内同时提供可直接作为 system prompt / Agent instructions 使用的 [机器人 Agent 文档](skills/xiy-llm-wiki/agents/agent.md)，包含人设、命令菜单、安全边界、`/ask` 与 `/llm` 规则，以及从当前仓库和 `~/.xiy/config.json` 解析并读取 Wiki 的完整流程。[openai.yaml](skills/xiy-llm-wiki/agents/openai.yaml) 仅作为支持 Skill Agent 元数据的 UI / harness 入口，并会引导机器人读取 `agent.md`。读取默认不修改或提交 Wiki；运行环境仍需允许 `git pull --ff-only` 更新 Git 元数据，严格文件只读 sandbox 应由外部任务预先同步 Wiki。
