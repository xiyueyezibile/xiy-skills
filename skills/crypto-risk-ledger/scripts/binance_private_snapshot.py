...
def load_config() -> Dict[str, object]:
    if not CONFIG.is_file():
        raise RuntimeError("Missing %s; run crypto_memory.py configure-binance" % CONFIG)
...
def binance_base_url(config: Dict[str, object]) -> str:
    binance = config.get("binance")
    if isinstance(binance, dict) and binance.get("baseUrl"):
        return str(binance["baseUrl"]).rstrip("/")
    return DEFAULT_BASE_URL


def signed_get(base_url: str, path: str, api_key: str, api_secret: str, extra_params: Optional[Dict[str, str]] = None) -> object:
...
def recent_user_trades(base_url: str, symbols: Set[str], api_key: str, api_secret: str) -> Dict[str, List[Dict[str, object]]]:
    result: Dict[str, List[Dict[str, object]]] = {}
    for symbol in sorted(symbols):
        response = signed_get(base_url, "/fapi/v1/userTrades", api_key, api_secret, {"symbol": symbol, "limit": "100"})
...
def main() -> int:
    try:
        ...
        account = signed_get(base_url, "/fapi/v2/account", api_key, api_secret)
        positions = signed_get(base_url, "/fapi/v2/positionRisk", api_key, api_secret)
