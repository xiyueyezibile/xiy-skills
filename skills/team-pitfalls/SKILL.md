...
前置检查不是一次性免检。任务执行中如果发现要进入新的稳定领域、页面链路、仓库模块、外部工具、数据系统、文档规范或其他高风险操作，且该范围没有被本轮已读取的 repo/domain 索引覆盖，应先暂停当前操作并补跑一次 `begin_task.py`，保留同一个 `--task-id`，追加新的 `--domain`。

补查触发条件：

- 新操作会修改代码、配置、数据、文档规范、外部系统或可见交付物。
- 新操作属于不同业务域、页面簇、端内链路、平台工具、验证方式或知识写入范围。
- 原始前置检查只覆盖了通用、仓库级或其他领域索引，无法支撑当前新操作的风险判断。
...
补查后仍按同一读取优先级：先读新返回的 repo/domain `index.md` 简介；只有简介、标题或当前新操作线索相关时，才打开对应正文。若补查命中并实际改变方案或风险判断，按正文条目记录 `使用次数`。如果只是同一领域内的普通子步骤、已读取索引明确覆盖的操作，或纯内部推理，不需要重复补查。
