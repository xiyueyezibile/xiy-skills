---
name: team-pitfalls
description: 团队踩坑与仓库业务知识收集器。只要用户提到“收集问题/踩坑/容易写错/沉淀经验/团队规范/业务黑话/术语映射/AI 找错被纠正”等，或本轮任务开始前需要先检查已有坑、结束后需要判断是否值得沉淀时，就必须调用本 skill。
---
...
只要本轮使用了这个 skill，不管是因为用户显式提到，还是因为当前任务明显涉及踩坑沉淀/纠错复盘，都必须执行下面两段：

1. **对话前检查**
   - 在正式回答、改文件、给方案之前，先读取索引和相关条目
   - 先做“这类坑之前有没有记过”的检查
   - 如果用户是在纠正上一轮输出，先做原则校验，再继续处理
2. **对话后复盘**
   - 在本轮任务准备结束时，再回看一次本轮对话和结果
   - 判断这次有没有新的通用坑、仓库黑话、纠错记录值得沉淀
...
当本轮任务满足以下任一情况时，在开始正常回答或执行前先调用本 skill 做“已有坑检查”：

- 用户显式提到 `team-pitfalls`
- 用户要求收集问题、沉淀经验、整理规范、沉淀业务黑话
- 用户正在纠正上一轮输出，或你已经知道当前任务很容易撞到历史坑
- 你准备开始处理某个仓库内的文档、规范、术语、README、commit、内部平台读取等高频易错问题
...
只要本轮已经调用过这个 skill，在准备结束本轮任务前，都要自动执行一次“是否值得沉淀”的复盘，不需要等用户再次提醒。
...
在输出任何正式结论、方案、代码、commit 信息或文档改动之前，先执行：

1. 判断是否命中仓库级场景
2. 读取对应索引：
   - 通用 [INDEX.md](references/INDEX.md)
   - 如命中仓库级，再读取 `references/repos/<repo-name>/INDEX.md`
3. 按“类型判定”只加载相关的 1~2 个通用文件
4. 如命中仓库级，再按需加载 `glossary.md` / `corrections.md`
5. 提取本轮最相关的已有结论，形成一份临时检查清单
6. 后续回答或执行过程中，优先遵循这份清单，避免重复踩坑
...
在本轮任务准备结束时，再执行一次简短复盘：

1. 回顾本轮用户是否纠正了你的输出、规则理解或业务含义
2. 判断有没有新的通用坑、仓库黑话、纠错记录值得沉淀
3. 如果值得记录，按上面的写入命令落盘
4. 如果不值得记录，也要在最终答复中明确说明：
   - 已调用 `team-pitfalls`
   - 已做结束前复盘
   - 本轮没有新增值得沉淀的条目
