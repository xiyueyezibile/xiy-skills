...
def load_config() -> Dict[str, object]:
    if not CONFIG.is_file():
        raise RuntimeError("Missing ~/.crypto/config.json; configure Binance credentials first")
    if CONFIG.stat().st_mode & 0o077:
        raise RuntimeError("Unsafe config permissions; run chmod 600 ~/.crypto/config.json")
    value = json.loads(CONFIG.read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise RuntimeError("Invalid ~/.crypto/config.json")
    return value
...
def public_request(path: str, params: Optional[Dict[str, str]] = None) -> object:
...
def signed_request(method: str, path: str, api_key: str, api_secret: str, params: Optional[Dict[str, str]] = None) -> object:
...
def symbol_rules(symbol: str) -> Dict[str, Decimal]:
    response = public_request("/fapi/v1/exchangeInfo")
...
def mark_price(symbol: str) -> Decimal:
    response = public_request("/fapi/v1/premiumIndex", {"symbol": symbol})
...
def prepare(args: argparse.Namespace) -> None:
    ...
    current_mark = mark_price(symbol)
...
def account_position_mode(api_key: str, api_secret: str) -> bool:
    response = signed_request("GET", "/fapi/v1/positionSide/dual", api_key, api_secret)
...
def active_symbol_positions(symbol: str, api_key: str, api_secret: str) -> List[Dict[str, object]]:
    response = signed_request("GET", "/fapi/v2/positionRisk", api_key, api_secret, {"symbol": symbol})
...
def active_symbol_algo_orders(symbol: str, api_key: str, api_secret: str) -> List[Dict[str, object]]:
    response = signed_request("GET", "/fapi/v1/openAlgoOrders", api_key, api_secret, {"symbol": symbol})
    if not isinstance(response, list):
        raise RuntimeError("Unexpected openAlgoOrders response")
    return [item for item in response if isinstance(item, dict)]


def set_leverage(symbol: str, leverage: int, api_key: str, api_secret: str) -> object:
    return signed_request("POST", "/fapi/v1/leverage", api_key, api_secret, {"symbol": symbol, "leverage": str(leverage)})


def set_margin_type(symbol: str, margin_type: str, api_key: str, api_secret: str) -> object:
    api_value = "CROSSED" if margin_type == "CROSS" else "ISOLATED"
    try:
        return signed_request("POST", "/fapi/v1/marginType", api_key, api_secret, {"symbol": symbol, "marginType": api_value})
...
def place_market(symbol: str, side: str, position_side: str, quantity: str, client_order_id: str, api_key: str, api_secret: str, reduce_only: bool = False) -> Dict[str, object]:
    params = {
        "symbol": symbol,
        "side": side,
    ...
    response = signed_request("POST", "/fapi/v1/order", api_key, api_secret, params)
    if not isinstance(response, dict):
        raise RuntimeError("Unexpected market order response")
    return response


def place_protection(symbol: str, side: str, position_side: str, order_type: str, trigger_price: str, client_algo_id: str, api_key: str, api_secret: str, quantity: Optional[str] = None) -> Dict[str, object]:
    params = {
...
def cancel_algo(algo_id: object, api_key: str, api_secret: str) -> None:
    if algo_id is None:
        return
    signed_request("DELETE", "/fapi/v1/algoOrder", api_key, api_secret, {"algoId": str(algo_id)})
...
def execute(args: argparse.Namespace) -> None:
    ...
    positions = active_symbol_positions(symbol, api_key, api_secret)
    if positions:
        raise RuntimeError("Existing symbol position detected; closePosition protection would not be order-exclusive")
    algo_orders = active_symbol_algo_orders(symbol, api_key, api_secret)
    if algo_orders:
    ...
    atomic_json(proposal_path(args.token), proposal)
    dual = account_position_mode(api_key, api_secret)
    position_side = entry_position_side(str(proposal["side"]), dual)
    leverage_result = set_leverage(symbol, int(proposal["leverage"]), api_key, api_secret)
    margin_result = set_margin_type(symbol, str(proposal["marginType"]), api_key, api_secret)
    prefix = "cns" + args.token[:12]
    try:
        entry = place_market(symbol, entry_order_side(str(proposal["side"])), position_side, str(proposal["quantity"]), prefix + "e", api_key, api_secret)
    except urllib.error.HTTPError as error:
    ...
    for index, level in enumerate(levels, start=1):
        if not isinstance(level, dict):
            ...
            continue
        try:
            take_profits.append(place_protection(symbol, protection_side, position_side, "TAKE_PROFIT_MARKET", str(level["triggerPrice"]), prefix + "t" + str(index), api_key, api_secret, str(level["quantity"])))
        except (RuntimeError, urllib.error.HTTPError, urllib.error.URLError) as error:
            tp_errors.append(str(error))
    try:
        stop_loss = place_protection(symbol, protection_side, position_side, "STOP_MARKET", str(proposal["stopLoss"]), prefix + "s", api_key, api_secret)
    except (RuntimeError, urllib.error.HTTPError, urllib.error.URLError) as error:
        sl_error = str(error)
    ...
    if stop_loss is None:
        for take_profit in take_profits:
            try:
                cancel_algo(take_profit.get("algoId"), api_key, api_secret)
            except (RuntimeError, urllib.error.HTTPError, urllib.error.URLError):
                pass
        try:
            emergency_close = place_market(symbol, protection_side, position_side, executed_quantity, prefix + "x", api_key, api_secret, reduce_only=True)
