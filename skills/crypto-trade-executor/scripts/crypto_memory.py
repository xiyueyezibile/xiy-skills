#!/usr/bin/env python3
"""Manage workspace-root .crypto credentials, trade events, and LLM Wiki skeleton."""
...
def configure_binance() -> None:
    ...
    write_binance_config(api_key, api_secret)
    print(json.dumps({"status": "configured", "config": str(CONFIG), "permissions": "0600"}, ensure_ascii=False))


def write_binance_config(api_key: str, api_secret: str, base_url: Optional[str] = None, source_env: Optional[Path] = None) -> None:
    ...
    if not isinstance(binance, dict):
        binance = {}
    binance.update({"apiKey": api_key, "apiSecret": api_secret})
    if base_url:
        binance["baseUrl"] = base_url.rstrip("/")
    payload["binance"] = binance
    payload["updatedAt"] = now_iso()
    if source_env is not None:
        payload["sourceEnv"] = str(source_env)
...
def import_binance_env(args: argparse.Namespace) -> None:
    initialize()
    source = Path(args.path).expanduser().resolve()
    if not source.is_file():
        raise ValueError("Env file not found: %s" % source)
    values = parse_env_file(source)
    api_key = values.get(args.api_key_var, "").strip()
    api_secret = values.get(args.api_secret_var, "").strip()
    if not api_key or not api_secret:
        raise ValueError("Missing %s or %s in %s" % (args.api_key_var, args.api_secret_var, source))
    base_url = ""
    for key in args.base_url_var:
        value = values.get(key, "").strip()
        if value:
            base_url = value
            break
    write_binance_config(api_key, api_secret, base_url or None, source)
    print(
        json.dumps(
            {
                "status": "configured",
                "config": str(CONFIG),
                "sourceEnv": str(source),
                "permissions": "0600",
...
def parser() -> argparse.ArgumentParser:
    ...
    import_env_parser = commands.add_parser("import-binance-env")
    import_env_parser.add_argument("--path", required=True, help="Path to a .env file containing Binance credentials")
    import_env_parser.add_argument("--api-key-var", default="BINANCE_API_KEY")
    import_env_parser.add_argument("--api-secret-var", default="BINANCE_API_SECRET")
    import_env_parser.add_argument("--base-url-var", action="append", default=["BINANCE_PROXY_URL", "BINANCE_BASE_URL", "BINANCE_FUTURES_BASE_URL"])
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        elif args.command == "import-binance-env": import_binance_env(args)
